...
full.nobias %>% select(-true, -truedn, -trueds, -dn, -ds) %>%
  na.omit() %>%
  group_by(ntaxa, bl, site, method) %>%
  summarize(mean.dnds = mean(dnds)) %>%
  spread(method, mean.dnds) %>%
  filter(!(is.infinite(FEL2)), !(is.infinite(FEL1))) %>% na.omit() -> nobias.compare.dnds
full.bias  %>% select(-true, -truedn, -trueds, -dn, -ds) %>%
  na.omit() %>%
  group_by(ntaxa, bl, site, method) %>%
  summarize(mean.dnds = mean(dnds)) %>%
  spread(method, mean.dnds) %>%
  filter(!(is.infinite(FEL2)), !(is.infinite(FEL1))) %>% na.omit() -> bias.compare.dnds

nobias.fel.dnds <-nobias.compare.dnds %>% ggplot(aes(x = FEL1, y = FEL2)) + geom_point() + facet_grid(ntaxa~bl) + geom_abline(slope=1, intercept=0, color="red", size=0.7) + xlab("dN, FEL1") + ylab("dN, FEL2") + ggtitle("No codon bias") + coord_cartesian(xlim=c(0,1), ylim=c(0,1))
bias.fel.dnds <- bias.compare.dnds %>% ggplot(aes(x = FEL1, y = FEL2)) + geom_point() + facet_grid(ntaxa~bl) + geom_abline(slope=1, intercept=0, color="red", size=0.7) + xlab("dNds, FEL1") + ylab("dNds, FEL2") + ggtitle("Codon bias") + coord_cartesian(xlim=c(0,1), ylim=c(0,1))

dnds.same.grid <- plot_grid(nobias.fel.dnds, bias.fel.dnds, nrow=1, labels=c("A", "B"), label_size=20)
save_plot(paste0(PLOTDIR, "fel12_dnds_scatter.pdf"), dnds.same.grid, base_width = 10, base_height=5)
